...
Define the retry policy standard for LaunchEdge so Next.js changes remain testable, deployable on Netlify, and easy to review.
...
- The retry policy requirement is observable and testable.
...
Keep implementation work focused on one retry policy outcome and record follow-up improvements separately.